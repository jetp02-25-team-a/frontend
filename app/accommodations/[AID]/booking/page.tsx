...
export default function BookingPage() {
  ...
  if (checkInDate) {
    ...
    checkOutDate = checkIn.toISOString().split('T')[0]; // 顯示用 YYYY-MM-DD
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      !guestName ||
      !guestEmail ||
      selectedRoomTypeId === null // 新增檢查確保房型ID存在
    ) {
      toast('請檢查所有預訂資訊是否完整且庫存足夠。');
      setLoading(false);
      return;
    }

    // 計算 API 用的 checkOutDate (ISO 格式)
    ...
    const payload = {
      ...
      // 這裡由於上面做了 selectedRoomTypeId === null 的檢查，故可以安全傳遞
      items: [{ roomTypeId: selectedRoomTypeId, quantity }],
    };

    try {
      const response = await apiFetch<BookingSuccessResponse>(`/m3/bookings`, {
        method: 'POST',
        headers: {
      ...
      const newBookingId = response.id; // 假設 API 成功回傳新訂單的 ID

      toast.success('🎉 訂單已建立成功！');

      // 🎯 成功跳轉到訂單詳情頁 (accommodations/booking/[BID]/page.tsx)
      router.push(`/accommodations/booking/${newBookingId}`);
    } catch (err) {
